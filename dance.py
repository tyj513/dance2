...
def difference(xy1, xy2, frames1, frames2, landmarks1, landmarks, ref_audio):     # all the joints we are using
    ...
    frame_height, frame_width, _ = frames1[0].shape
    output_width = frame_width * 2  # 两个视频并排
    ...
    temp_video_path = f'{OUTPUT_DIR}/temp_output.mov'
    video = VideoWriter(temp_video_path, fourcc, 24.0, (output_width, frame_height), isColor=True)
    ...
    final_output_path = f'{OUTPUT_DIR}/output_with_audio.mp4'
    ffmpeg_command = [
        'ffmpeg',
        '-i', temp_video_path,
        '-i', ref_audio,
    ...
    return score, final_output_path
...
def convert_to_same_framerate(clip):
    ...
    subprocess.run(f"ffmpeg {EXIST_FLAG} -i {clip} -filter:v fps=24 {clip_24}")
    return clip_24
...
def find_sound_offset(ref_wav, comparison_wav):
    start_position = 0
    command = f"C:\\Praat.exe --run crosscorrelate.praat {ref_wav} {comparison_wav} {start_position} {SEARCH_INTERVAL}"
    
    result = subprocess.run(command, shell=True, capture_output=True, text=True)
    
    if result.returncode != 0:
        print("Error running Praat script:")
        print(result.stderr)
        raise RuntimeError("Praat script failed")
    
    offset_str = result.stdout.strip()
    print(f"Raw Praat output: {offset_str}")
    
    # Clean the output
    offset_str = ''.join(char for char in offset_str if char.isdigit() or char in '.-')
    
    print(f"Cleaned offset string: {offset_str}")
    
    try:
        return abs(float(offset_str))
    except ValueError as e:
        print(f"Error converting offset to float. Cleaned offset: {offset_str}")
        raise e
# --------------------------------------------------------- COMPUTE SYNC ------------------------------------------------------------------------
...
score, output_video = difference(xy_dancer1, xy_dancer2, dancer1_frames, dancer2_frames, dancer1_landmarks, dancer2_landmarks, ref_clip_wav)
print(f"\n You are {score:.2f} % in sync with your model dancer!")
print(f"Output video with audio saved as: {output_video}")
